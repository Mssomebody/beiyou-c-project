...
# -*- coding: utf-8 -*-
"""
五星级不加权联邦学习 + 贝叶斯自动找最优 μ
直接使用预处理数据中的 Valor_norm 列（Z-score），无需额外归一化
自动日志保存至 logs/ 目录，便于可视化
"""
...
# ============================================================
# 自定义 Dataset：直接使用预处理好的 Valor_norm 列
# ============================================================
class BarcelonaDatasetValorNorm(Dataset):
    def __init__(self, data_path, window_size=28, predict_size=4,
        ...
        """
        直接使用预处理好的 Valor_norm 列，无需额外归一化
        """
...
class FederatedTrainer:
    ...
    def train_full(self, mu: float, train_loaders: Dict, val_loaders: Dict, test_loaders: Dict) -> Tuple[float, float]:
        ...
        if best_model:
            ...
            # 保存模型，文件名包含窗口大小和节点数，便于识别
            model_filename = f"model_window{self.config.window_size}_nodes{len(self.config.nodes)}_mu{mu:.6f}.pth"
            model_path = self.config.output_dir / model_filename
            torch.save(model.state_dict(), model_path)
            self.logger.info(f"  最佳模型保存至: {model_path}")
...
def main():
    ...
    parser.add_argument('--window_size', type=int, default=28, help='输入窗口长度（时间步）')
    parser.add_argument('--output_dir', type=str, default=None, help='输出目录（用于保存模型和结果）')
    ...
    if args.mu is not None:
        ...
        logger.info(f"\n最终 sMAPE: {best_smape:.2f}%")
        logger.info(f"最佳模型已保存在: {config.output_dir}/model_window{config.window_size}_nodes{len(config.nodes)}_mu{args.mu:.6f}.pth")
        return args.mu
