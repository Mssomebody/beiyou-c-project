...
LOG_FILE = LOG_DIR / f"fusion_learnable_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class FusionLSTM(nn.Module):
    def __init__(self, input_dim_barcelona, input_dim_tsinghua, hidden_dim, num_layers, output_dim, dropout):
        ...
        # 可学习时段权重（4个时段）
        self.segment_weights = nn.Parameter(torch.ones(4))  # 初始为1

    def forward(self, x, is_barcelona):
        if is_barcelona:
            x = self.proj(x)                     # (batch, 4, 5)
            # 应用可学习时段权重
            weight = self.segment_weights.view(1, -1, 1)  # (1,4,1)
            x = x * weight
        out, _ = self.lstm(x)
        last_out = out[:, -1, :]
