...
# -*- coding: utf-8 -*-
"""
粒度融合 + 知识迁移加权 (E4) - 损失加权版
- 根据节点聚类类型动态选择时段权重
- 将时段权重应用于损失函数（加权MSE），而非输入特征
- 自动日志
"""
...
LOG_FILE = LOG_DIR / f"fusion_loss_weighted_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ...
        for client_id, loader in client_loaders.items():
            ...
            for _ in range(self.config.local_epochs):
                ...
                for x, y in loader:
                    ...
                    # --- 损失加权（时段权重）---
                    loss_elem = (output - y) ** 2   # (batch, 4)
                    weight = self.get_segment_weights(client_id)   # (4,)
