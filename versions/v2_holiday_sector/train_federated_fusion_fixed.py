...
class FederatedTrainer:
    ...
    def train_full(self, mu, client_loaders, test_loaders):
        ...
        for round_num in range(1, self.config.rounds + 1):
            model = self.train_round(model, client_loaders, mu)
...
def main():
    ...
    # 转换为 DataLoader（每个客户端一个 DataLoader，batch_size=64）
    client_loaders = {}
    for cid, dataset in all_clients.items():
        client_loaders[cid] = DataLoader(dataset, batch_size=64, shuffle=True, drop_last=True)
    test_loaders = {}
    for cid, dataset in test_clients.items():
        test_loaders[cid] = DataLoader(dataset, batch_size=64, shuffle=False, drop_last=True)
