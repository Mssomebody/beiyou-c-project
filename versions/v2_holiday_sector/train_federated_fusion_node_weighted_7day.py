...
"""
节点加权联邦学习 (E2) - 7天窗口版
基于 E3 基础粒度融合脚本，在服务器聚合时添加节点权重（空间维度）
自动日志保存至 logs/，无根目录污染
支持多天输入
"""
...
LOG_FILE = LOG_DIR / f"fusion_node_weighted_7day_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
sys.stdout = Logger(LOG_FILE)

# ============================================================
# 加载节点权重（全局平均）
# ============================================================
NODE_WEIGHTS_FILE = PROJECT_ROOT / "results/barcelona_clustering/node_weights_for_federated.json"
NODE_WEIGHTS = None
if NODE_WEIGHTS_FILE.exists():
    with open(NODE_WEIGHTS_FILE, 'r') as f:
    ...
    print(f"Loaded node weights for {len(NODE_WEIGHTS)} 个节点")
else:
    print(f"Warning: node weights file not found: {NODE_WEIGHTS_FILE}，将使用均匀权重")
...
class FederatedTrainer:
    def __init__(self, config, node_weights):
        self.config = config
    ...
    def train_round(self, model, client_loaders, mu):
        ...
        if self.node_weights is not None:
            total_weighted = sum(self.node_weights.get(str(cid), 1.0) * size
    ...
    def train_full(self, mu, client_loaders, test_loaders):
        ...
        torch.save(model.state_dict(), "model_7day_e2_5nodes.pth")
        print("模型已保存为 model_7day_e2_5nodes.pth")
        return smape
...
# 数据加载函数
# ============================================================
def load_barcelona_clients(nodes, data_dir, window_days=7):
    clients = {}
    norm_params_dict = {}
    for node in nodes:
...
def main():
    ...
    print("加载巴塞罗那节点...")
    barcelona_clients, test_clients = load_barcelona_clients(config.barcelona_nodes, config.barcelona_data_dir, window_days=config.window_days)
    ...
    trainer = FederatedTrainer(config, NODE_WEIGHTS)
