...
# -*- coding: utf-8 -*-
"""
粒度融合 + 知识迁移加权 (E4) - 7天窗口版
基于原 E4 脚本修改，支持7天输入窗口
"""
...
LOG_FILE = LOG_DIR / f"fusion_time_weighted_7day_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class FederatedTrainer:
    ...
    def get_segment_weights(self, client_id):
        if client_id.startswith('barcelona'):
            ...
            w = self.all_weights[wtype].to(self.device)  # shape (4,)
            # 重复 window_days 次，得到 (window_days*4,)
            w = w.repeat(self.config.window_days)
            return w
        else:
            return self.all_weights['mixed'].to(self.device).repeat(self.config.window_days)

    def apply_time_weights(self, x, client_id):
        w = self.get_segment_weights(client_id)  # (seq_len,)
    ...
    def train_full(self, mu, client_loaders, test_loaders):
        ...
        torch.save(model.state_dict(), "model_7day_e4_5nodes.pth")
        print("模型已保存为 model_7day_e4_5nodes.pth")
        return smape


# ============================================================
# 数据加载函数（传递 window_days）
# ============================================================
