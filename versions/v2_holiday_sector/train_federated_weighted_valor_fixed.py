...
"""
五星级联邦学习 + 贝叶斯自动找最优 μ
使用原始能耗 Valor，在线 MinMax 归一化（参数从训练集计算）
"""
...
if WEIGHTS_PATH.exists():
    ...
    print(f"Loaded node weights for {len(NODE_WEIGHTS)} nodes")
...
# ============================================================
# 自定义 Dataset：使用原始能耗 Valor，在线 MinMax 归一化（参数从训练集计算）
# ============================================================
...
class FederatedTrainer:
    ...
    def train_round(self, model: nn.Module, train_loaders: Dict[int, DataLoader], mu: float) -> nn.Module:
        client_weights = []
        client_sizes = []
        print(f"train_loaders keys: {list(train_loaders.keys())}")
        for client_id, loader in train_loaders.items():
            print(f"Processing client {client_id}, loader dataset length: {len(loader.dataset)}")
            ...
            # 收集客户端信息
        client_ids = list(train_loaders.keys())  # 需要在循环外获取，但这里可以重新从 train_loaders 获取
        # 由于在循环中已经收集了 client_ids，我们可以在循环中同时收集
        # 修改循环，在收集权重时也记录 client_id
        # 为了简单，我们重新从 train_loaders 获取 client_ids
        ...
        if self.node_weights is not None:
            ...
            for cid, size in zip(client_ids, client_sizes):
                w = self.node_weights.get(str(cid), 1.0)
                print(f"节点 {cid}: 权重={w:.6f}, 数据量={size}")
            print(f"total_weighted = {total_weighted:.2f}")
            global_weights = {}
            ...
            print(f"各节点聚合权重系数: {[self.node_weights.get(str(cid), 1.0) * size / total_weighted for cid, size in zip(client_ids, client_sizes)]}")
        else:
            # 均匀聚合
            global_weights = {}
            for key in client_weights[0].keys():
