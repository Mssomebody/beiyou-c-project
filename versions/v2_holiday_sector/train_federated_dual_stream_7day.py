...
# -*- coding: utf-8 -*-
"""
双流粒度融合：巴塞罗那7天窗口（28步） + 清华4步
- 流A：处理巴塞罗那28步输入（LSTM）
- 流B：处理清华4步输入（LSTM）
- 融合层：将两个流最后时间步的输出拼接后全连接
"""
...
LOG_FILE = LOG_DIR / f"dual_stream_7day_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class DualStreamLSTM(nn.Module):
    ...
    def forward(self, x_b, x_t):
        # x_b: (batch, 28, 7)
        # x_t: (batch, 4, 5)
        _, (h_b, _) = self.lstm_b(x_b)   # h_b: (num_layers, batch, hidden_dim)
        _, (h_t, _) = self.lstm_t(x_t)   # h_t: (num_layers, batch, hidden_dim)
        # 取最后一层隐藏状态
        last_h_b = h_b[-1]   # (batch, hidden_dim)
        last_h_t = h_t[-1]   # (batch, hidden_dim)
        concat = torch.cat([last_h_b, last_h_t], dim=1)   # (batch, hidden_dim*2)
    ...
    def train_round(self, model, client_loaders, mu):
        ...
        for client_id, loader in client_loaders.items():
            ...
            for _ in range(self.config.local_epochs):
                ...
                for x, y in loader:
                    if is_barcelona:
                        # 巴塞罗那数据本身就是28步，直接作为x_b；清华数据需要从 loader 中获取？但 loader 中只有 x，没有清华数据
                        # 这里设计：每个客户端只提供一种数据，巴塞罗那节点只提供 x_b，清华节点只提供 x_t
                        # 但模型需要两个输入，对于巴塞罗那节点，x_t 怎么来？我们需要在训练时，对于巴塞罗那节点，清华输入用零填充或平均？
                        # 简化：巴塞罗那节点训练时，清华输入用零填充（表示无清华数据）
                        # 清华节点训练时，巴塞罗那输入用零填充
