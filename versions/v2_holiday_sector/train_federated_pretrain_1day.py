...
- 输入特征：7维（能耗+部门+周末+节假日）
- 联邦学习（FedAvg + FedProx），41节点
- 支持早停、学习率调度、梯度裁剪
- 输出模型到 results/two_stage/model_fed_pretrain_1day.pth
"""
...
# 输出
OUTPUT_MODEL = PROJECT_ROOT / "results" / "two_stage" / "model_fed_pretrain_1day.pth"
LOG_FILE = PROJECT_ROOT / "results" / "two_stage" / "pretrain_1day_log.txt"

# 窗口参数（1天 = 4步）
WINDOW_SIZE = 4   # 输入过去4个时段（1天）
PREDICT_SIZE = 4  # 预测未来4个时段（1天）
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="一天窗口滑动窗口预训练")
