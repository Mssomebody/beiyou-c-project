...
# -*- coding: utf-8 -*-
"""
粒度融合 + 知识迁移加权 (E4) - 动态节点权重版
- 根据节点聚类类型（工业/商业/住宅）动态选择时段权重
- 自动日志
"""
...
LOG_FILE = LOG_DIR / f"fusion_time_weighted_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class FederatedTrainer:
    def __init__(self, config, all_weights, node_types):
        ...
        self.all_weights = all_weights          # {'mixed': tensor, '4g_like': ..., '5g_like': ...}
        self.node_types = node_types            # {node: 'mixed'/'4g_like'/'5g_like'}
    ...
    # 计算逐元素MSE损失
    loss_elem = (output - y) ** 2   # shape: (batch, 4)
    # 获取时段权重（4个值）
    weight = self.get_segment_weights(client_id)   # shape: (4,)
    # 加权平均损失
    ...
    def evaluate_smape(self, model, test_loaders):
        ...
        with torch.no_grad():
            for client_id, loader in test_loaders.items():
                for x, y in loader:
                    x, y = x.to(self.device), y.to(self.device)
                    x = self.apply_time_weights(x, client_id)   # 测试集也应用相同权重
