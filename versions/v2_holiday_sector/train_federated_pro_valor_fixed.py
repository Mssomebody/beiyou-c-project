...
# -*- coding: utf-8 -*-
"""
五星级不加权联邦学习 + 贝叶斯自动找最优 μ（使用原始能耗 Valor，MinMax归一化）
修复：评估时反归一化到原始空间计算 sMAPE，确保测试集目标值超出训练集范围时仍正确
"""
...
# ============================================================
class BarcelonaDatasetValor(Dataset):
...
class DataLoaderFactory:
    ...
    def load_all_loaders(cls, config: FedConfig):
        ...
        return train_loaders, val_loaders, test_loaders, norm_params_dict
...
class FederatedTrainer:
    def __init__(self, config: FedConfig, logger: Logger, norm_params_dict: Dict):
        self.config = config
        self.logger = logger
        self.device = torch.device(config.device)
        self.norm_params_dict = norm_params_dict   # 存储每个节点的归一化参数
    ...
    def inverse_normalize(self, y_norm, node_id):
        """将归一化后的目标值反归一化到原始能耗"""
        params = self.norm_params_dict.get(node_id)
        if params is None:
            return y_norm
        y_min = params['y_min']
        y_max = params['y_max']
        ...
        with torch.no_grad():
            for node_id, loader in test_loaders.items():
                for x, y in loader:
                    ...
                    output_norm = model(x)
                    # 反归一化到原始空间
                    pred_raw = self.inverse_normalize(output_norm, node_id)
                    target_raw = self.inverse_normalize(y, node_id)
                    all_preds_raw.append(pred_raw.cpu().numpy())
                    all_targets_raw.append(target_raw.cpu().numpy())
        
        all_preds = np.concatenate(all_preds_raw, axis=0)
        all_targets = np.concatenate(all_targets_raw, axis=0)
...
def objective(trial: optuna.Trial, config: FedConfig, logger: Logger, norm_params_dict: Dict) -> float:
    mu = trial.suggest_float('mu', config.mu_min, config.mu_max)
    logger.info(f"Trial {trial.number}: μ={mu:.6f}")
    
    train_loaders, val_loaders, test_loaders, _ = DataLoaderFactory.load_all_loaders(config)
    
    trainer = FederatedTrainer(config, logger, norm_params_dict)
...
def main():
    ...
    logger.info("="*70)
    logger.info("联邦学习 + 贝叶斯自动找最优 μ（使用原始能耗 Valor，MinMax归一化）")
    logger.info("="*70)
    ...
    # 加载数据（同时获取归一化参数）
    train_loaders, val_loaders, test_loaders, norm_params_dict = DataLoaderFactory.load_all_loaders(config)
    ...
    def wrapped_objective(trial):
        return objective(trial, config, logger, norm_params_dict)
