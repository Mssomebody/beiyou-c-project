...
# 使用五节点七天窗口模型（已存在）
model_path = PROJECT_ROOT / "versions" / "v2_holiday_sector" / "model_7day_5nodes.pth"
...
# 复用之前的数据集类（注意 window_size=28）
from test_dataset import MinMaxBarcelonaDataset

node_id = 8001
test_file = PROJECT_ROOT / "data" / "processed" / "barcelona_ready_v1" / f"node_{node_id}" / "test.pkl"
dataset = MinMaxBarcelonaDataset(test_file, node_id, node_minmax, window_size=28, predict_size=4)
...
print(f"七天窗口模型（五节点）在节点 {node_id} 的真实 sMAPE: {smape:.2f}%")
