# day3_multivariate_fl_data.py
# 第3天：多变量时序数据 + 联邦学习Non-IID数据准备
...
for site_id in range(5):
    ...
    ax.scatter(sample['temp'], sample['power'], c=colors[site_id], alpha=0.5, s=10)
...
for site_id in range(5):
    ...
    ax.scatter(df['temp'], df['power'], c=colors[site_id], alpha=0.5, s=10, label=f'Site {site_id}')
