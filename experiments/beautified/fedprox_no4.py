...
def run_single_experiment(mu, clients, num_rounds=5):
    print(f"\n{'='*60}")
    ...
    for round_idx in tqdm(range(num_rounds), desc=f"μ={mu}"):
        client_models = []
        round_train_loss = 0
        ...
        checkpoint_path = os.path.join(checkpoint_dir, f"fedprox_mu{mu}_round{round_idx+1}.pth")
        torch.save({
            'round': round_idx + 1,
...
def main():
    ...
    for mu in mus:
        train_losses, test_losses = run_single_experiment(mu, clients, num_rounds=5)
        all_results[mu] = {
            'train': train_losses,
            'test': test_losses,
    ...
    for i, mu in enumerate(mus):
        ...
        ax1.plot(all_results[mu]['train'], linewidth=2.5, color=colors[i], label=label)
    ...
    for i, mu in enumerate(mus):
        label = f'μ={mu}' + (" (FedAvg)" if mu == 0 else "")
        ax2.plot(all_results[mu]['test'], linewidth=2.5, color=colors[i], label=label)
